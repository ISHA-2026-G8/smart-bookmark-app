// "use client"

// import { createBrowserClient } from "@supabase/ssr"

// export default function AuthButton() {
//   const supabase = createBrowserClient(
//     process.env.NEXT_PUBLIC_SUPABASE_URL!,
//     process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
//   )

//   const handleLogin = async () => {
//     await supabase.auth.signInWithOAuth({
//       provider: "google",
//       options: {
//         redirectTo: `${window.location.origin}/auth/callback`,
//       },
//     })
//   }

//   return (
//     <button
//       onClick={handleLogin}
//       className="bg-black text-white px-8 py-4 rounded-lg text-lg font-medium hover:bg-gray-800 transition-colors"
//     >
//       Sign in with Google
//     </button>
//   )
// }



"use client"

import { createBrowserClient } from "@supabase/ssr"

export default function AuthButton() {
  const supabase = createBrowserClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
  )

  const handleLogin = async () => {
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback`,
      },
    })
  }

  return (
    <button
      onClick={handleLogin}
      style={{
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        gap: '10px',
        background: '#4f7ef8',
        color: 'white',
        fontWeight: '700',
        fontSize: '15px',
        padding: '14px 32px',
        borderRadius: '50px',
        border: 'none',
        cursor: 'pointer',
        fontFamily: "'DM Sans', sans-serif",
        width: '100%',
        maxWidth: '320px',
        transition: 'all 0.2s ease',
        boxShadow: '0 4px 16px rgba(79, 126, 248, 0.35)',
        letterSpacing: '0.01em',
      }}
      onMouseEnter={e => {
        (e.target as HTMLButtonElement).style.background = '#3a6ef0'
        ;(e.target as HTMLButtonElement).style.boxShadow = '0 6px 20px rgba(79, 126, 248, 0.45)'
      }}
      onMouseLeave={e => {
        (e.target as HTMLButtonElement).style.background = '#4f7ef8'
        ;(e.target as HTMLButtonElement).style.boxShadow = '0 4px 16px rgba(79, 126, 248, 0.35)'
      }}
    >
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="white"/>
        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="white" fillOpacity="0.85"/>
        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="white" fillOpacity="0.7"/>
        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="white" fillOpacity="0.9"/>
      </svg>
      Sign in with Google
    </button>
  )
}